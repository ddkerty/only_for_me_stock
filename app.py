# -*- coding: utf-8 -*-
# Combined app.py V2.1 (FRED Key Removed, Tech Analysis Call Fixed)
    ...
    try:
        ...
        if os.path.exists(dotenv_path):
            ...
            if fmp_env_key:
             ...
             if not secrets_available: sidebar_status.error(".env 파일 없음, Secrets에도 FMP 키 없음."); final_status_message_displayed = True
             elif fmp_key_loaded: sidebar_status.empty()
    except Exception as e:
...
with st.sidebar:
    ...
    st.markdown("---")
    if page == "📊 종합 분석":
        st.header("⚙️ 종합 분석 설정"); ticker_input = st.text_input("종목 티커", "AAPL", key="main_ticker", help="해외(예: AAPL) 또는 국내(예: 005930 - FMP는 .KS 제외)", disabled=not comprehensive_analysis_possible)
        analysis_years = st.select_slider("분석 기간 (년)", [1, 2, 3, 5, 7, 10], 2, key="analysis_years", disabled=not comprehensive_analysis_possible); st.caption(f"과거 {analysis_years}년 데이터 분석")
        forecast_days = st.number_input("예측 기간 (일)", 7, 90, 30, 7, key="forecast_days", disabled=not comprehensive_analysis_possible); st.caption(f"향후 {forecast_days}일 예측")
        num_trend_periods_input = st.number_input("재무 추세 분기 수", 2, 12, 4, 1, key="num_trend_periods", disabled=not comprehensive_analysis_possible); st.caption(f"최근 {num_trend_periods_input}개 분기 재무 추세 계산")
        st.divider(); st.subheader("⚙️ 예측 세부 설정 (선택)")
        changepoint_prior_input = st.slider("추세 변화 민감도 (Prophet)", 0.001, 0.5, 0.05, 0.01, "%.3f", help="클수록 과거 추세 변화에 민감 (기본값: 0.05)", key="changepoint_prior", disabled=not comprehensive_analysis_possible); st.caption(f"현재 민감도: {changepoint_prior_input:.3f}")
        ...
        quantity = st.number_input("보유 수량", 0, step=1, key="quantity", disabled=not comprehensive_analysis_possible); st.caption("평단가 입력 시 리스크 트래커 분석 활성화"); st.divider()
    elif page == "📈 기술 분석":
        st.header("⚙️ 기술 분석 설정"); bb_window = st.number_input("볼린저밴드 기간 (일)", 5, 50, 20, 1, key="bb_window")
        bb_std = st.number_input("볼린저밴드 표준편차 배수", 1.0, 3.0, 2.0, 0.1, key="bb_std", format="%.1f"); st.caption(f"현재 설정: {bb_window}일 기간, {bb_std:.1f} 표준편차"); st.divider()
...
# ============== 📊 종합 분석 탭 ==============
if page == "📊 종합 분석":
    st.title("📊 종합 분석 결과 (FMP API 기반)"); st.markdown("기업 정보, 재무 추세, 예측, 리스크 트래커 제공."); st.markdown("---")
    ...
    if analyze_button_main:
        ...
        cp_prior = st.session_state.get('changepoint_prior', 0.05); avg_p = st.session_state.get('avg_price', 0.0); qty = st.session_state.get('quantity', 0)
        if not ticker: results_placeholder.warning("종목 티커 입력 필요.")
        else:
            ticker_proc = ticker.strip().upper()
            if '.KS' in ticker_proc or '.KQ' in ticker_proc: original_ticker = ticker_proc; ticker_proc = ticker_proc.split('.')[0]; results_placeholder.info(f"국내 티커 감지: {original_ticker} -> {ticker_proc} (FMP용)")
            with st.spinner(f"{ticker_proc} 종합 분석 중..."):
                try:
                    ...
                    if results and isinstance(results, dict):
                        if "error" in results: results_placeholder.error(f"분석 실패: {results['error']}")
                        else:
                            results_placeholder.empty() # 이전 메시지 지우기
                            if results.get("warn_high_mape"): m = results.get("mape", "N/A"); mape_value_str = m if isinstance(m, str) else (f"{m:.1f}%" if isinstance(m, (int, float)) else "N/A"); st.warning(f"🔴 모델 정확도 낮음 (MAPE {mape_value_str}). 예측 신뢰도 주의!")
                            with results_placeholder:
                                st.header(f"📈 {ticker_proc} 분석 결과 (민감도: {cp_prior:.3f})")
                                st.subheader("요약 정보"); col1, col2, col3 = st.columns(3)
                                col1.metric("현재가", f"${results.get('current_price', 'N/A')}"); col2.metric("분석 시작일", results.get('analysis_period_start', 'N/A')); col3.metric("분석 종료일", results.get('analysis_period_end', 'N/A'))
                                ...
                                if fundamentals and isinstance(fundamentals, dict) and fundamentals.get("시가총액", "N/A") != "N/A":
                                    ...
                                    if summary != "N/A": with st.expander("회사 요약 보기"): st.write(summary)
                                ...
                                trend_data_map = {"영업이익률(%)": ('operating_margin_trend', 'Op Margin (%)', "{:.2f}%"),"ROE(%)": ('roe_trend', 'ROE (%)', "{:.2f}%"),"부채비율": ('debt_to_equity_trend', 'D/E Ratio', "{:.2f}"),"유동비율": ('current_ratio_trend', 'Current Ratio', "{:.2f}")}
                                for i, title in enumerate(tab_titles):
                                    with tabs[i]:
                                        data_key, col_name, style_format = trend_data_map[title]; trend_data = results.get(data_key)
                                        if trend_data and isinstance(trend_data, list) and len(trend_data) > 0:
                                            try:
                                                ...
                                                if col_name in df_trend.columns: st.line_chart(df_trend[[col_name]]); with st.expander("데이터 보기"): st.dataframe(df_trend[[col_name]].style.format({col_name: style_format}), use_container_width=True)
                                ...
                                if stock_chart_fig: st.plotly_chart(stock_chart_fig, use_container_width=True); else: st.warning("주가 차트 생성 실패 (종합).")
                                st.divider()
                                st.subheader("시장 심리 분석"); col_news, col_fng = st.columns([2, 1])
                                with col_news:
                                    st.markdown("**📰 뉴스 감정 분석 (FMP + TextBlob)**"); news_sentiment = results.get('news_sentiment', ["정보 없음."])
                                    if isinstance(news_sentiment, list) and len(news_sentiment) > 0:
                                        st.info(news_sentiment[0]) # 헤더
                                        if len(news_sentiment) > 1: with st.expander("뉴스 목록 보기"): [st.write(f"- {line}") for line in news_sentiment[1:]]
                                ...
                                if isinstance(forecast_data_list, list) and len(forecast_data_list) > 0:
                                    ...
                                    try:
                                        df_fcst = pd.DataFrame(forecast_data_list); df_fcst['ds'] = pd.to_datetime(df_fcst['ds']); df_fcst_display = df_fcst.sort_values("ds").iloc[-10:].copy(); df_fcst_display['ds'] = df_fcst_display['ds'].dt.strftime('%Y-%m-%d')
                                        format_dict_fcst = {col: "{:.2f}" for col in ['yhat', 'yhat_lower', 'yhat_upper'] if col in df_fcst_display.columns}; st.dataframe(df_fcst_display[['ds'] + list(format_dict_fcst.keys())].style.format(format_dict_fcst), use_container_width=True)
                                    except Exception as e: st.error(f"예측 데이터 표시 오류: {e}")
                                cv_plot_path = results.get('cv_plot_path');
                                if cv_plot_path and os.path.exists(cv_plot_path): st.markdown("**📉 교차 검증 결과 (MAPE)**"); try: st.image(cv_plot_path, caption="MAPE (낮을수록 정확)") except Exception as img_e: st.warning(f"CV 이미지 로드 실패: {img_e}")
                                ...
                                if avg_p > 0 and isinstance(forecast_data_list, list) and len(forecast_data_list) > 0:
                                    try:
                                        df_pred = pd.DataFrame(forecast_data_list); required_fcst_cols = ['ds', 'yhat_lower']; valid_pred = False
                                        if all(col in df_pred.columns for col in required_fcst_cols): df_pred['ds'] = pd.to_datetime(df_pred['ds'], errors='coerce'); df_pred['yhat_lower'] = pd.to_numeric(df_pred['yhat_lower'], errors='coerce'); df_pred.dropna(subset=['ds', 'yhat_lower'], inplace=True); if not df_pred.empty: valid_pred = True
                                        if valid_pred:
                                            df_pred['평단가'] = avg_p; df_pred['리스크 여부'] = df_pred['yhat_lower'] < avg_p; df_pred['예상 손실률'] = np.where((df_pred['리스크 여부']) & (avg_p != 0), ((df_pred['yhat_lower'] - avg_p) / avg_p) * 100, 0).fillna(0)
                                            if qty > 0: df_pred['예상 손실액'] = np.where(df_pred['리스크 여부'], (df_pred['yhat_lower'] - avg_p) * qty, 0).fillna(0); else: df_pred['예상 손실액'] = 0
                                            risk_days = df_pred['리스크 여부'].sum()
                                            if risk_days > 0:
                                                valid_loss_pct = df_pred.loc[df_pred['리스크 여부'], '예상 손실률'].dropna(); max_loss_pct = valid_loss_pct.min() if not valid_loss_pct.empty else 0
                                                if qty > 0: valid_loss_amt = df_pred.loc[df_pred['리스크 여부'], '예상 손실액'].dropna(); max_loss_amt = valid_loss_amt.min() if not valid_loss_amt.empty else 0; else: max_loss_amt = 0
                                            else: max_loss_pct, max_loss_amt = 0, 0
                                            st.markdown("##### 리스크 요약"); col_r1, col_r2, col_r3 = st.columns(3); col_r1.metric("⚠️ < 평단가 일수", f"{risk_days}일 / {days}일"); col_r2.metric("📉 Max 손실률", f"{max_loss_pct:.2f}%");
                                            if qty > 0: col_r3.metric("💸 Max 손실액", f"${max_loss_amt:,.2f}")
                                            if risk_days > 0: st.warning(f"{days}일 예측 중 **{risk_days}일** 평단가(${avg_p:.2f}) 하회 가능성."); else: st.success(f"{days}일간 평단가(${avg_p:.2f}) 하회 가능성 낮음.")
                                            st.markdown("##### 평단가 vs 예측 구간 비교"); fig_risk = go.Figure(); plot_cols_risk = {'yhat_lower': 'rgba(0,100,80,0.2)', 'yhat_upper': 'rgba(0,100,80,0.2)', 'yhat': 'rgba(0,100,80,0.6)'}
                                            df_plot_risk = df_pred[['ds'] + list(plot_cols_risk.keys())].copy(); [df_plot_risk[col].update(pd.to_numeric(df_plot_risk[col], errors='coerce')) for col in plot_cols_risk]; df_plot_risk.dropna(subset=['ds'] + list(plot_cols_risk.keys()), how='any', inplace=True)
                                            if not df_plot_risk.empty:
                                                ...
                                                fig_risk.add_hline(y=avg_p, line_dash="dot", line_color="red", annotation_text=f"평단가: ${avg_p:.2f}", annotation_position="bottom right"); df_risk_periods = df_pred[df_pred['리스크 여부']]
                                                ...
                                                if risk_days > 0: with st.expander(f"리스크 예측일 상세 데이터 ({risk_days}일)"): df_risk_days_display = df_pred[df_pred['리스크 여부']].copy(); df_risk_days_display['ds'] = df_risk_days_display['ds'].dt.strftime('%Y-%m-%d'); cols_show = ['ds', 'yhat_lower', '평단가', '예상 손실률']; formatters = {"yhat_lower":"{:.2f}", "평단가":"{:.2f}", "예상 손실률":"{:.2f}%"}; if qty > 0 and '예상 손실액' in df_risk_days_display.columns: cols_show.append('예상 손실액'); formatters["예상 손실액"] = "${:,.2f}"; st.dataframe(df_risk_days_display[cols_show].style.format(formatters), use_container_width=True)
                                            else: st.info("차트 표시에 필요한 유효한 예측 데이터 부족.")
                                        else: st.info("리스크 분석 위한 유효 데이터 없음.")
                                    except Exception as risk_calc_err: st.error(f"리스크 트래커 계산/표시 중 오류 발생: {risk_calc_err}"); logging.error(f"Risk tracker error: {traceback.format_exc()}")
                                elif avg_p <= 0: st.info("⬅️ 사이드바에서 '평단가' 입력 시 리스크 분석 결과 확인 가능.")
                                else: st.warning("예측 데이터가 유효하지 않아 리스크 분석 수행 불가.")
                                st.divider()
                                st.subheader("🧐 자동 분석 결과 요약 (참고용)"); summary_points = [] # 요약 로직 시작
                                if isinstance(forecast_data_list, list) and forecast_data_list:
                                    try:
                                        ...
                                        if all(col in df_pred_summary.columns for col in ['yhat', 'yhat_lower', 'yhat_upper']):
                                             ...
                                             lower = df_pred_summary["yhat_lower"].min() if df_pred_summary['yhat_lower'].notna().any() else 'N/A'; upper = df_pred_summary["yhat_upper"].max() if df_pred_summary['yhat_upper'].notna().any() else 'N/A'
                                ...
                                if isinstance(news_sentiment, list) and len(news_sentiment) > 0 and ":" in news_sentiment[0]:
                                    try: score_part = news_sentiment[0].split(":")[-1].strip(); avg_score = float(score_part); sentiment_desc = "긍정적" if avg_score > 0.05 else "부정적" if avg_score < -0.05 else "중립적"; summary_points.append(f"- **뉴스:** 평균 감성 {avg_score:.2f}, **{sentiment_desc}** 분위기.")
                                ...
                                if fundamentals and isinstance(fundamentals, dict):
                                    per = fundamentals.get("PER", "N/A"); sector = fundamentals.get("업종", "N/A"); parts = [f"PER {per}"] if per != "N/A" else []; parts.extend([f"업종 '{sector}'"] if sector != "N/A" else []);
                                    if parts: summary_points.append(f"- **기본 정보:** {', '.join(parts)}."); else: summary_points.append("- 기본 정보: 주요 지표(PER, 업종) 없음.")
                                ...
                                try:
                                    for key in trend_labels:
                                        ...
                                        if trend_list and isinstance(trend_list, list): last_item = trend_list[-1]; value_key = trend_value_keys[key]; value = last_item.get(value_key)
                                ...
                                if avg_p > 0 and 'df_pred' in locals() and not df_pred.empty: # 리스크 요약 조건 확인
                                    if risk_days > 0: summary_points.append(f"- **리스크:** {days}일 중 **{risk_days}일** 평단가(${avg_p:.2f}) 하회 가능성 (Max 손실률: **{max_loss_pct:.2f}%**)."); else: summary_points.append(f"- **리스크:** 예측 기간 내 평단가(${avg_p:.2f}) 하회 가능성 낮음.")
                                elif avg_p > 0: summary_points.append(f"- 리스크: 평단가(${avg_p:.2f}) 입력됨, 예측 데이터 부족/오류로 분석 불가.")
                                if summary_points: st.markdown("\n".join(summary_points)); st.caption("⚠️ **주의:** 자동 생성된 요약이며 투자 결정의 근거가 될 수 없습니다."); else: st.write("분석 요약 생성 불가.")
                            # --- 결과 표시 영역 끝 ---
                ...
                except Exception as e: error_traceback = traceback.format_exc(); logging.error(f"종합 분석 메인 로직 실행 오류: {e}\n{error_traceback}"); results_placeholder.error(f"앱 실행 중 오류 발생: {e}")
    else: # 종합 분석 버튼 클릭 전
...
# ============== 📈 기술 분석 탭 ==============
    ...
    # ... (기술 분석 탭 코드 - 이전 답변 내용 참조하여 FMP 호출 및 처리 구현 필요) ...
    ...
    today = datetime.now().date(); default_start_date = today - relativedelta(months=3); min_date_allowed = today - relativedelta(years=5)
    ...
    with col3:
        available_intervals = {"일봉": "1day", "1시간": "1hour", "15분": "15min"}; interval_help_fmp = """데이터 간격 선택 (FMP 기준):\n- FMP 플랜 따라 지원 간격 다름.\n- 분봉 데이터 유료 플랜 필요 가능성.\n* 조회 기간 제한도 FMP 플랜 따라 다름."""
        interval_display = st.selectbox("데이터 간격", list(available_intervals.keys()), key="tech_interval_display", help=interval_help_fmp); interval = available_intervals[interval_display]

    bb_window_val = st.session_state.get('bb_window', 20); bb_std_val = st.session_state.get('bb_std', 2.0)
    ...
    if analyze_button_tech:
        if not ticker_tech: st.warning("종목 티커를 입력해주세요.")
            ...
            if '.KS' in ticker_processed_tech or '.KQ' in ticker_processed_tech: original_ticker_tech = ticker_processed_tech; ticker_processed_tech = ticker_processed_tech.split('.')[0]; st.info(f"국내 티커 감지: {original_ticker_tech} -> {ticker_processed_tech} (FMP용)")
            df_tech = pd.DataFrame(); st.session_state['error_shown_tech'] = False
            ...
            with st.spinner(f"{ticker_processed_tech} 데이터 로딩 및 처리 중 (FMP)..."):
                try:
                    ...
                    if interval == "1day":
                        ...
                        rename_map = {'date': 'Date', 'open': 'Open', 'high': 'High', 'low': 'Low', 'close': 'Close', 'volume': 'Volume'} # FMP 응답 확인 필요
                    if fmp_data and isinstance(fmp_data, list):
                        df_tech = pd.DataFrame(fmp_data)
                        if not df_tech.empty:
                            df_tech = df_tech.rename(columns=rename_map); date_col_name = rename_map.get('date', 'Date')
                            if date_col_name in df_tech.columns:
                                df_tech[date_col_name] = pd.to_datetime(df_tech[date_col_name], errors='coerce'); df_tech = df_tech.set_index(date_col_name).sort_index()
                                for col in ['Open', 'High', 'Low', 'Close', 'Volume']:
                                    if col in df_tech.columns: df_tech[col] = pd.to_numeric(df_tech[col], errors='coerce')
                                df_tech.dropna(subset=['Open', 'High', 'Low', 'Close'], inplace=True)
                            else: st.error(f"FMP 응답 날짜 컬럼 '{date_col_name}' 없음."); df_tech = pd.DataFrame()
                        else: st.warning(f"FMP 데이터 '{ticker_processed_tech}' ({interval_display}) 없음."); df_tech = pd.DataFrame()
                    elif fmp_data is None: st.error(f"FMP 데이터 로딩 오류 (API 결과 None)."); df_tech = pd.DataFrame()
                    else: st.warning(f"FMP 데이터 '{ticker_processed_tech}' ({interval_display}) 가져오기 실패 (API 결과 빈 리스트)."); df_tech = pd.DataFrame()

                    if df_tech.empty:
                        if not st.session_state.get('error_shown_tech'): st.error(f"❌ 데이터 조회/처리 실패."); st.session_state['error_shown_tech'] = True
                    else:
                        st.session_state['error_shown_tech'] = False; logging.info(f"FMP 데이터 처리 완료 ({ticker_processed_tech}, {interval_display}). {len(df_tech)} 행.")
                        st.caption(f"조회 기간 (FMP): {df_tech.index.min()} ~ {df_tech.index.max()}")
                        required_cols_tech = ['Open', 'High', 'Low', 'Close', 'Volume']; missing_cols_tech = [col for col in required_cols_tech if col not in df_tech.columns]
                        if missing_cols_tech: st.error(f"❌ 필수 컬럼 누락: {missing_cols_tech}."); st.dataframe(df_tech.head())
                        else:
                            ...
                            st.subheader(f"📌 {ticker_processed_tech} 기술적 분석 통합 차트 ({interval_display})"); chart_tech = plot_technical_chart(df_calculated, ticker_processed_tech)
                            if chart_tech and chart_tech.data: st.plotly_chart(chart_tech, use_container_width=True); else: st.warning("차트 생성 실패/표시 데이터 없음.")
                            st.subheader("📄 최근 데이터 (계산된 지표 포함)"); display_cols = ['Open', 'High', 'Low', 'Close', 'Volume', 'VWAP', 'MA20', 'Upper', 'Lower', 'RSI', 'MACD', 'MACD_signal', 'MACD_hist']
                            display_cols_exist = [col for col in display_cols if col in df_calculated.columns]; format_dict = {col: "{:.2f}" for col in display_cols_exist if col != 'Volume'}; format_dict['Volume'] = "{:,.0f}"
                            ...
                            if not df_calculated.empty:
                                latest_row = df_calculated.iloc[-1].copy(); signal_messages = []
                                try: signal_messages.extend(interpret_technical_signals(latest_row, df_context=df_calculated))
                                except Exception as e: st.warning(f"기본 기술적 시그널 해석 오류: {e}", icon="⚠️")
                                if signal_messages: [st.info(msg) for msg in signal_messages]
                ...
                except requests.exceptions.RequestException as req_err: st.error(f"FMP API 요청 실패: {req_err}"); logging.error(f"FMP API request error (Tech Tab): {req_err}")
                except EnvironmentError as env_err: st.error(f"FMP API 키 설정 오류: {env_err}"); logging.error(f"FMP API key error (Tech Tab): {env_err}")
                except Exception as e: st.error(f"기술적 분석 처리 중 오류 발생: {type(e).__name__} - {e}"); logging.error(f"Tech analysis tab error: {traceback.format_exc()}")
    else: st.info("종목 티커, 기간, 데이터 간격 등을 설정한 후 '기술적 분석 실행' 버튼을 클릭하세요.")

# --- 앱 정보 ---
st.sidebar.markdown("---"); st.sidebar.info("종합 주식 분석 툴 (FMP API) | 정보 제공 목적 (투자 조언 아님)"); st.sidebar.markdown("📌 [개발기 보러가기](https://technut.tistory.com/1)", unsafe_allow_html=True); st.sidebar.caption("👨‍💻 기술 기반 주식 분석 툴 개발기")